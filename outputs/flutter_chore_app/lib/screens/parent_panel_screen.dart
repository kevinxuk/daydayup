...
class _ParentPanelScreenState extends State<ParentPanelScreen> {
  ...
  Future<void> _authenticate() async {
    ...
    if (profile.parentPassword.isEmpty) {
      context.read<ChoreProvider>().updateProfile(UserProfile(
        childName: _nameCtrl.text.isEmpty ? '小朋友' : _nameCtrl.text,
      ...
      body: Center(
        child: Container(
          ...
          child: Column(mainAxisSize: MainAxisSize.min, children: [
            ...
            Text(profile.parentPassword.isEmpty ? '设置家长密码' : '输入家长密码', style: const TextStyle(fontSize: 18, fontWeight: FontWeight.bold)),
            const SizedBox(height: 8),
            TextField(controller: _nameCtrl, decoration: const InputDecoration(labelText: '孩子姓名', border: OutlineInputBorder())),
            const SizedBox(height: 8),
            TextField(controller: _classCtrl, decoration: const InputDecoration(labelText: '班级', border: OutlineInputBorder())),
            const SizedBox(height: 8),
            TextField(controller: _pwdCtrl, obscureText: true, decoration: const InputDecoration(labelText: '密码', border: OutlineInputBorder())),
            const SizedBox(height: 16),
            ElevatedButton(onPressed: _authenticate, style: ElevatedButton.styleFrom(backgroundColor: Colors.blueAccent), child: const Text('确认')),
          ]),
        ),
      ),
    );
        ...
        child: Column(
          children: [
            Container(
              ...
              child: Column(
                children: [
                  const CircleAvatar(radius: 40, backgroundColor: Colors.white, child: Icon(Icons.child_care, size: 48, color: Colors.blueAccent)),
                  const SizedBox(height: 12),
                  Text(profile.childName, style: const TextStyle(fontSize: 20, fontWeight: FontWeight.bold, color: Colors.white)),
                  if (profile.className.isNotEmpty) Text(profile.className, style: const TextStyle(color: Colors.white70)),
                  const SizedBox(height: 12),
            ...
            Container(margin: const EdgeInsets.symmetric(horizontal: 16), decoration: BoxDecoration(color: Colors.white, borderRadius: BorderRadius.circular(12)), child: Column(
              children: [
    ...
    await showDialog(context: context, builder: (_) => AlertDialog(
      ...
      actions: [
        ...
        TextButton(onPressed: () {
          final p = context.read<ChoreProvider>().profile;
          context.read<ChoreProvider>().updateProfile(UserProfile(childName: ctrl.text, className: p.className, parentPassword: p.parentPassword, totalEarned: p.totalEarned, streakDays: p.streakDays, lastActiveDate: p.lastActiveDate));
          Navigator.pop(_);
        }, child: const Text('保存')),
      ],
    ));
  ...
  Future<void> _editClass() async {
    ...
    await showDialog(context: context, builder: (_) => AlertDialog(
      ...
      actions: [
        ...
        TextButton(onPressed: () {
          final p = context.read<ChoreProvider>().profile;
          context.read<ChoreProvider>().updateProfile(UserProfile(childName: p.childName, className: ctrl.text, parentPassword: p.parentPassword, totalEarned: p.totalEarned, streakDays: p.streakDays, lastActiveDate: p.lastActiveDate));
          Navigator.pop(_);
        }, child: const Text('保存')),
      ],
    ));
